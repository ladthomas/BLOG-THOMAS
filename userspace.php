...
<a  style="color: #FFFA47;"  href="userspace.php?id=<?= $_GET['id'] ?>">Actualités</a>
 <?php
...
while ($dataReqShowArticles = $ReqShowArticles->fetch()){
    ...
    <?php
    if($dataReqShowArticles["posteur"] == $data['id'] ){
    ?>
    
    <a style="position:relative;right:-180px;height:20px;" href="suppression.php?idi=<?= $dataReqShowArticles['id'] ?>&com=<?= $dataReqShowArticles['posteur'] ?>&nom=<?= $data['nom']?>&img=<?= $dataReqShowArticles['images'] ?>">Delete</a>
